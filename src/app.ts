...
class CurrentIssueGetter {
  ...
  async get () {
    const x = this.context.name === 'issues' ? this.context.payload.issue : await this.context.github.issues.get(this.context.repo({ issue_number: this.context.issue().number })).event.payload.issue
    console.log(x)
    return x
  }
}
